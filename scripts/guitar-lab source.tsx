...
function YoutubeMiniPlayer({ link, onClose, onSaveBookmark, onAddImages }) {
  ...
  const loadCaptureFile = (file, wasLocked) => {
    ...
    reader.onload = (ev) => {
      ...
      setCapHasImage(true);
      if (!wasLocked) showCapMessage('Image collée — ajuste le cadre puis tape « Capturer »');
    };
    reader.onerror = () => showCapMessage('Image illisible — réessaie');
  ...
  const handleCapturePasteEvent = (e) => {
    ...
    if (!item) { showCapMessage('Aucune image dans le presse-papiers'); return; }
    loadCaptureFile(item.getAsFile(), capFrameLocked);
  };

  // Bouton "Coller" explicite : lit directement le presse-papiers (plus fiable que le geste sur iPad)
  const captureFromClipboardButton = async () => {
    try {
      if (!navigator.clipboard?.read) {
        showCapMessage('Collage direct indisponible — utilise le geste Coller ci-dessous');
        return;
      }
      ...
      showCapMessage('Aucune image dans le presse-papiers');
    } catch (err) {
      showCapMessage('Collage indisponible — utilise le geste Coller ci-dessous');
    }
  };
        ...
        {captureOpen && (
          <div className="p-3 bg-gray-850 border-b border-gray-700 space-y-2">
            <p className="text-[10px] text-gray-400 leading-relaxed">
              1. Mets la vidéo en pause sur l'accord voulu &nbsp;•&nbsp; 2. Capture d'écran iPad &nbsp;•&nbsp; 3. Colle-la ci-dessous
            </p>
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <label className="px-2 py-1 bg-gray-700 hover:bg-gray-600 border border-gray-600 rounded text-[10px] font-semibold cursor-pointer">
                📁 Importer
              ...
              {capHasImage && (
                <span className={`ml-auto text-[10px] px-1.5 py-0.5 rounded ${capFrameLocked ? 'bg-purple-500/20 text-purple-300' : 'bg-amber-500/20 text-amber-300'}`}>
                  {capFrameLocked ? '🔒 verrouillé — coller = capture auto' : '🔓 ajuste le cadre'}
                </span>
              )}
            </div>
